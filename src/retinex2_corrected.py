...
def apply_constant_illumination_from_sr(
    log_img: np.ndarray,      # (H,W,3), normalized log image
    sr_map: np.ndarray,       # (H,W,3), spectral-ratio / ISD map
    L: np.ndarray,            # (H,W), scalar illumination estimate
    target_illum: 0.8,      # desired constant illumination level
    eps: float = 1e-6,
) -> tuple[np.ndarray, dict]:
    """
    Shift each pixel along the SR/ISD direction so that the scalar
    illumination map is normalized to a constant target value.
    """

    # normalize SR map to unit direction
    n = np.linalg.norm(sr_map, axis=2, keepdims=True)
    unit_sr = np.divide(sr_map, n, out=np.zeros_like(sr_map), where=n > eps)

    # scalar gain needed to move illumination to constant level
    L_n = np.clip(L / (np.percentile(L, 99.5) + eps), 1e-3, 1.0)
    gain = (target_illum + eps) / (L_n + eps)
    # convert multiplicative gain to normalized-log shift magnitude
    alpha = np.log(gain) / np.log(65535.0)

    # shift in log space
    log_out = log_img + alpha[..., None] * unit_sr

    return log_out, {
        "gain": gain,
        "alpha": alpha,
        "target_illum": target_illum,
    }
# -----------------------------
# CLI
# -----------------------------
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--square", action='store_true', help="Center crops image to 512x512." )
    return p
...
def main() -> None:
    ...
    print(f"    Max: {img_bgr.max(axis=(0,1))} | Min: {img_bgr.min(axis=(0,1))}")
    if args.square:
    ...
    if args.use_model:
        ...
        print(f" Predicated Map | Shape: {isd_map_rgb.shape} | Dtype: {isd_map_rgb.dtype}")
        print(f" Predicated Map | Max: {isd_map_rgb.max(axis=(0,1))} | Min: {isd_map_rgb.min(axis=(0,1))}")
    else:
    ...
    log_img_constant_rgb, _ = apply_constant_illumination_from_sr(
                                                        log_img=log_img_rgb,
                                                        sr_map=isd_map_rgb,
                                                        L=L,
                                                        target_illum=1.1
    ...
    lin_img_constant_rgb = log_normalized_to_linear16(log_img_constant_rgb)
    save_image_float(str(out_dir / "result_constant_L.png"), lin_img_constant_rgb, channel_order="rgb")
